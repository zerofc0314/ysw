...
/**
 * 文章分页查询方法
 * @param string $tag  查询标签，以字符串方式传入,例："cid:1,2;field:post_title,post_content;limit:0,8;order:post_date desc,listorder desc;where:id>0;get_child:1"<br>
 * 	ids:调用指定id的一个或多个数据,如 1,2,3<br>
 * 	cid:数据所在分类,可调出一个或多个分类数据,如 1,2,3 默认值为全部,在当前分类为:'.$cid.'<br>
 * 	field:调用post指定字段,如(id,post_title...) 默认全部<br>
 * 	limit:数据条数,默认值为10,可以指定从第几条开始,如3,8(表示共调用8条,从第3条开始)<br>
 * 	order:排序方式，如：post_date desc<br>
 * 	where:查询条件，字符串形式，和sql语句一样
 *      get_child:是否查询子级，如果不设置的话则不查询子级。
 * @param int $pagesize 每页条数.
 * @param string $pagetpl 以字符串方式传入,例："{first}{prev}{liststart}{list}{listend}{next}{last}"
...
function sp_sql_posts_paged($tag, $pagesize = 20, $pagetpl = '{first}{prev}{liststart}{list}{listend}{next}{last}') {
    ...
    if (isset($tag['cid'])) {
        $cid = array(); //初始化定义
        if (isset($tag['get_child'])) {
            $child_id = sp_get_child_terms($tag['cid'], TRUE, 'term_id');
            $tag['cid'] = $child_id ?  implode_array($child_id) : $tag['cid'] ; 
        }
        $where['term_id'] = array('in', $tag['cid']);
    }
 ...
 * 返回指定分类下的子分类
 * @param int $term_id 分类id
 * @param  bool $get_child 是否获取所有子分类
 * @return array 返回指定分类下的子分类
 */
function sp_get_child_terms($term_id, $get_child = FALSE, $filed = '') {
    ...
    if ($get_child == TRUE) {
        foreach ($terms as $key => $value) {
            $terms[$key]['child'] = sp_get_child_terms($value['term_id'], TRUE, $filed);
        }
    }
    if ($filed) {
        $temp_terms = array();
        foreach ($terms as $v) {
            if (isset($v[$filed])) {
                $temp_terms[] = $v['child'] ? $v['child'] : $v[$filed];
            }
        }
    }

    return isset($temp_terms) && $temp_terms ? $temp_terms : $terms;
}
...
function implode_array($array) {
    ...
    if (is_array($array)) {
        foreach ($array as $key => $value) {
            $temp_array[$key] = is_array($value) ? implode_array($value) : $value;
        }
    }
    return is_array($array) ? implode(',', $temp_array) : $array;
}
